...
// Revisions:   02/24/03 - PH Modified for my own devious purposes
...
/*****************************************************************************************
*/
char *MD5Checksum::GetMD5(char *filename)
{
    static char md5str[256];
    ...
    if (fp) {
        ...
        fclose(fp);
        strcpy(md5str, md5.GetMD5());
        if (strlen(md5str) != 32) {
            md5str[0] = '\0';   // an error occurred
        }
    } else {
        md5str[0] = '\0';       // an error occurred
    }
...
*****************************************************************************************/
DWORD MD5Checksum::RotateLeft(DWORD x, int n)
{
...
NOTES:			None
*****************************************************************************************/
void MD5Checksum::FF( DWORD& A, DWORD B, DWORD C, DWORD D, DWORD X, DWORD S, DWORD T)
{
	DWORD F = (B & C) | (~B & D);
	A += F + X + T;
	A = RotateLeft(A, S);
...
NOTES:			None
*****************************************************************************************/
void MD5Checksum::GG( DWORD& A, DWORD B, DWORD C, DWORD D, DWORD X, DWORD S, DWORD T)
{
	DWORD G = (B & D) | (C & ~D);
	A += G + X + T;
	A = RotateLeft(A, S);
...
NOTES:			None
*****************************************************************************************/
void MD5Checksum::HH( DWORD& A, DWORD B, DWORD C, DWORD D, DWORD X, DWORD S, DWORD T)
{
	DWORD H = (B ^ C ^ D);
	A += H + X + T;
	A = RotateLeft(A, S);
...
NOTES:			None
*****************************************************************************************/
void MD5Checksum::II( DWORD& A, DWORD B, DWORD C, DWORD D, DWORD X, DWORD S, DWORD T)
{
	DWORD I = (C ^ (B | ~D));
	A += I + X + T;
	A = RotateLeft(A, S);
...
/*****************************************************************************************
FUNCTION:		MD5Checksum::ByteToDWord
DETAILS:		private
...
*****************************************************************************************/
void MD5Checksum::ByteToDWord(DWORD* Output, BYTE* Input, UINT nLength)
{
	//entry invariants
//	ASSERT( nLength % 4 == 0 );
...
/*****************************************************************************************
FUNCTION:		MD5Checksum::Transform
DETAILS:		protected
...
/*****************************************************************************************
FUNCTION:		MD5Checksum::Init - PH 03/13/03
DESCRIPTION:	Initialises member data
ARGUMENTS:		None
NOTES:			This routine called by the constructor, but it may be called again
                to re-initialize the object if desired.
*****************************************************************************************/
...
/*****************************************************************************************
FUNCTION:		MD5Checksum::DWordToByte
DETAILS:		private
...
*****************************************************************************************/
void MD5Checksum::DWordToByte(BYTE* Output, DWORD* Input, UINT nLength )
{
	//entry invariants
//	ASSERT( nLength % 4 == 0 );
...
Revisions:      03/14/03 - PH allow nInputLen > 32k
*****************************************************************************************/
void MD5Checksum::Update( BYTE* Input,	ULONG nInputLen )
{
    //Compute number of bytes mod 64
    ULONG nIndex = (ULONG)((m_nCount[0] >> 3) & 0x3F);
